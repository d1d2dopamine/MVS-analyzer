...
{
    public const string FormulaVersion = "MVS-1.3.0";
    public const string FormulaSpecification = "score=100*power^.30*falseAlarm^.25*robustness^.20*repeatability^.15*coverage^.10;rawValueScenario;globalRankTest;tracks=location,variability;powerPerTrack;scorePerTrack;falseAlarmRobustnessRepeatabilityCoverageSharedAcrossTracks;repeatability=splitHalfGroupMedianAgreement;coverage=bootstrapIntervalCoverage;candidate=fpr<=.075&&trackPower>=.70&&trackScore>=60;candidateEvaluatedPerTrack;maxCandidatesPerTrack=4;nearMissReported;effect=cliffsDelta;interval=percentileBootstrap400;equivalence=tostOnBootstrapDelta;verdict=difference|equivalent|insufficient|not_applicable;mde=interpolatedFromEffectGridPerTrack@power.80;effectGrid=1.00,1.02,1.05,1.10,1.20;inflatedFpr=nullGridPointAboveAlpha";
    ...
    public static List<OutputArtifact> Export(string folder, string runId, string project, string projectDescription, string projectMode, string dataset, string datasetHash, AnalysisData data, List<CalibrationRow> calibration, List<ResultRow> results, AppSettings settings, int calibrationRepetitions, IEnumerable<OutputArtifact> existing, string calibrationSource = "same_dataset")
    {
        ...
        if (settings.AutoExportManifest)
        {
            ...
            var manifest = new
            {
                application = "MVS Analyzer", version = "1.5.0", engineVersion = AnalysisEngine.EngineVersion, runId, created = DateTimeOffset.Now,
    ...
    internal static string ResultsCsv(IEnumerable<ResultRow> rows) { var s = new StringBuilder("metric,group_summary,range,global_p,effect_cliffs_delta,effect_low,effect_high,equivalence_p,verdict,mde,calibrated_fpr,fpr_inflated,calibrated_power,robustness,repeatability,coverage,mvs_score,applicable,candidate,near_miss,tracks,track_powers,track_scores,track_mdes,candidate_tracks\r\n"); foreach (var r in rows) s.AppendLine(string.Join(',', Csv(r.Metric), Csv(r.GroupSummary), N(r.MedianRange), N(r.PValue), N(r.Effect), N(r.EffectLow), N(r.EffectHigh), N(r.EquivalenceP), Csv(r.Verdict), N(r.Mde), N(r.Fpr), r.FprInflated.ToString(), N(r.Power), N(r.Robustness), N(r.Repeatability), N(r.Coverage), N(r.Score), r.Applicable.ToString(), r.Candidate.ToString(), r.NearMiss.ToString(), Csv(Join(r.Tracks)), Csv(Join(r.TrackPowers)), Csv(Join(r.TrackScores)), Csv(Join(r.TrackMdes)), Csv(r.CandidateTracks))); return s.ToString(); }
    internal static string CalibrationCsv(IEnumerable<CalibrationRow> rows) { var s = new StringBuilder("metric,calibrated_fpr,fpr_inflated,calibrated_power,mde,power_curve,robustness,repeatability,coverage,mvs_score,applicable,tracks,track_powers,track_scores,track_mdes,track_curves\r\n"); foreach (var r in rows) s.AppendLine(string.Join(',', Csv(r.Metric), N(r.Fpr), r.FprInflated.ToString(), N(r.Power), N(r.Mde), Csv(r.PowerCurve), N(r.Robustness), N(r.Repeatability), N(r.Coverage), N(r.Score), r.Applicable.ToString(), Csv(Join(r.Tracks)), Csv(Join(r.TrackPowers)), Csv(Join(r.TrackScores)), Csv(Join(r.TrackMdes)), Csv(Join(r.TrackCurves)))); return s.ToString(); }
    ...
    // Track columns are pipe joined so the CSV schema does not change shape when the number
    // of tracks changes. A reader that ignores them sees exactly the file it saw before.
