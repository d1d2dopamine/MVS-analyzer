...
`calibration.csv` preserves leading legacy columns and appends counts/bounds/statuses. `calibration_tracks.csv` is long format: one metric/track row, power, Wilson bounds, MCSE, score, MDE, MDE status and failures. `calibration_recommendations.csv` is the 1.5 development recommendation layer: it records metric family, family eligibility, null-control status, power interval, the best-metric reference interval, candidate membership and recommendation quality. A small-budget blank MDE does **not** mean “effects above 20% are invisible”.
...
`run_manifest.json` records processing, actual calibration tracks, decision family, legacy candidate sets per track, the 1.5 `developmentRecommendations` summaries, method specification/hash, plugin metadata, warnings and output checksums. Calibration state is written/copied **before** the manifest and included in its checksums. `--force` records both current and calibration input hashes. Input bytes are not embedded in the manifest. The execution-environment fingerprint is recorded alongside the original calibration environment stored in the state; different arithmetic environments do not promise bitwise replay.

Pseudonyms are deterministic truncated hashes and **not strong anonymization**. Dataset names may remain inside a calibration state for operational replay; inspect state files before external sharing. Paths, group names and project descriptions can also disclose information. During the calibration-recommendation checkpoint, `candidateSet` and `candidateSetsByTrack` remain legacy 1.4 result labels for compatibility. The new family-aware recommendation set is in `calibration_recommendations.csv` and `developmentRecommendations`. Imported/exported text beginning with a spreadsheet formula character is escaped in new core CSV exports. Review third-party report templates separately.
