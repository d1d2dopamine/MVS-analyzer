...
internal sealed class ColabBridge : IDisposable
{
    ...
    private async Task ServeAsync(TcpClient client)
    {
        ...
        using (var timeout = CancellationTokenSource.CreateLinkedTokenSource(cancellation.Token))
        {
            ...
            try
            {
                ...
                if (route.Length != 3 || route[0] != "v1" || !ColabSessionStore.HexKey(route[1]) || !new[] { "job", "status", "request" }.Contains(route[2]))
                { await Respond(stream, new(Array.Empty<byte>(), Status: 404), origin, timeout.Token); return; }
                byte[] body = Array.Empty<byte>();
                if (request[0] == "POST")
                {
                    if (!int.TryParse(fields.GetValueOrDefault("Content-Length"), out int length) || length < 0 || length > 16 * 1024 * 1024) return;
                ...
                { reply = new(Encoding.UTF8.GetBytes("{\"error\":\"The MVS request was rejected. Check the local job and notebook version.\"}"), Status: 400); }
    ...
    {
        string status = reply.Status switch { 200 => "OK", 204 => "No Content", 403 => "Forbidden", 404 => "Not Found", _ => "Bad Request" };
