...
The intended use is choosing a metric before you analyse, or checking that a conclusion does not rest on one lucky metric. Picking the winner after looking at the same data you will draw conclusions from costs up to four times the nominal false-alarm rate — see Validation for the measured number.
> [!NOTE]
...
⚡ Quick start
Download the build
Latest release → `MVS_Analyzer_<version>_win-x64.zip`. Unzip, run `MVS_Analyzer.exe`. Windows 10 or later, x64; no .NET installation required. The build is not code-signed, so SmartScreen warns on first run — verify the SHA-256 against `SHA256SUMS.txt` in the release, or build from source below.
Run from source
```powershell
...
```
First five minutes
Pick a language on first launch (English / Русский — changeable later in Settings).
Data → Guided example, or load `examples/demo_three_groups.csv`.
Settings → scenario, outlier rate, missing rate, effect multiplier, seed.
Calibration → run it. Run → start the analysis.
Results → read the verdict card, then `results.csv` and `run_manifest.json` in the run folder.
Audit → point it at the output folder and confirm the run verifies.
Want to see the app catch its own mistakes? Load `examples/MVS_stress_test.csv` — Control / Shift +6 % / Noise. Level metrics and spread metrics point at different groups there, and the results card is required to say so instead of calling it agreement.
...
```
Data. One variable, 2–10 independent groups, ≥ 4 entities per group, ≥ 6 valid measurements per entity (configurable). Entity-level metrics are computed once.
Calibration. For every metric, two worlds are simulated from your own rows:
...
Run. The configuration is frozen, the input file is hashed, and the analysis is executed against the real group labels.
Results. One sentence first — which metric, which group is higher, by how much — then the statistics behind it, then the files.
<details>
...
Full protocol, figure guide and the honest list of what it does not prove: `docs/BENCHMARK.md`. Optional real-recording stage: `benchmark_data/README.md`.
---
...
Full guide, including what determinism now means across platforms: `docs/REMOTE.md`.
...
```
Any other naming (or a semicolon + decimal-comma export from a lab device) is handled by an import profile from a plugin — see `examples/lab_device_ru_win1251.csv` and docs/DATA_FORMAT.md.
---
🧱 Plugins (data only)
...
Three ready-made packs live in this repository: `plugin-report-pack-source`, `plugin-lab-pack-source`, `plugin-stress-pack-source`. Full format reference: docs/PLUGINS.md.
---
⌨️ Keyboard
...
Selecting a metric on the same data you report inflates the error rate. Under a pure null, the chance that at least one of the ten metrics comes out significant is 0.205, not 0.05. Use split calibration, or fix the metric before looking. See docs/METHODS.md.
...
The weights are judgement, not evidence — quantified by the sensitivity analysis in `validation/`.
...
The experiments, thresholds and current results are in docs/VALIDATION.md, frozen in advance in docs/PREREGISTRATION.md. Independent replication is welcome: datasets, seeds and an independent reference implementation are in the repository for exactly that.
...
See CHANGELOG.md for the full history — including the two bugs that version 1.3.2 exists to fix.
---
❓ FAQ
<details>
...
---
🤝 Contributing
Bug reports and pull requests are welcome — start with CONTRIBUTING.md, and read docs/METHODS.md first if the change touches statistics. One rule above all others: if the score definition changes, the formula version, its hash and `FormulaHash` test must change with it, in the same commit.
Bugs and ideas → Issues
Security and plugin-safety reports → SECURITY.md
Behaviour in the community → CODE_OF_CONDUCT.md
---
📌 Citation
If MVS Analyzer influenced a published result, cite it with the metadata in CITATION.cff and include the `formula.hash` and `engineVersion` from your `run_manifest.json` — that pair identifies exactly how the numbers were produced.
---
⚖️ License
MIT © d1d2dopamine
---
...
Программа рассчитана на то, что метрику выбирают до анализа — либо на проверку того, что вывод не держится на одной удачной метрике. Если выбирать победителя по тем же данным, по которым потом делается вывод, цена ошибки вырастает до четырёх раз против номинальной — измеренное число см. в Валидации.
> [!NOTE]
...
⚡ Быстрый старт
Скачать сборку
Последний релиз → `MVS_Analyzer_<версия>_win-x64.zip`. Распакуйте и запустите `MVS_Analyzer.exe`. Нужна Windows 10+ x64, устанавливать .NET не требуется. Сборка не подписана, поэтому при первом запуске SmartScreen покажет предупреждение — сверьте SHA-256 архива с `SHA256SUMS.txt` из релиза или соберите сами.
...
Первые пять минут
На первом запуске выберите язык (меняется потом в «Настройках»).
«Данные» → «Пример», либо загрузите `examples/demo_three_groups.csv`.
«Настройки» → сценарий, выбросы, пропуски, множитель эффекта, seed.
«Калибровка» → «Запуск».
«Результаты» → карточка вердикта, затем `results.csv` и `run_manifest.json`.
«Аудит» → укажите папку вывода и убедитесь, что прогон проверяется.
Хотите увидеть, как программа ловит сама себя? Загрузите `examples/MVS_stress_test.csv` — Control / Shift +6 % / Noise. Метрики уровня и метрики разброса там указывают на разные группы, и карточка результатов обязана это сказать, а не называть согласием.
---
🔬 Как это работает
```mermaid
flowchart LR
...
```
Данные. Одна переменная, 2–10 независимых групп, ≥ 4 объектов в группе, ≥ 6 годных измерений на объект (настраивается).
Калибровка. Для каждой метрики строятся два мира из ваших же строк:
...
Запуск. Конфигурация фиксируется, входной файл хешируется, расчёт идёт по настоящим меткам групп.
Результаты. Сначала одно предложение — какая метрика, какая группа выше и на сколько процентов, — потом статистика, потом файлы.
<details>
...
Чем этот балл не является. У него нет единиц измерения: все компоненты безразмерны, показатели степени в сумме дают 1, то есть это взвешенное среднее геометрическое в диапазоне [0, 100]. Шкала порядковая: «метрика A выше метрики B» — осмысленно, «на 8 баллов лучше» — нет. Порог `score ≥ 60` — это утверждение о расстоянии на порядковой шкале, и он снимается в 1.6.0. Подробно: docs/METHODS.md.
...
Полный протокол, разбор графиков и честный список того, что бенчмарк не доказывает: `docs/BENCHMARK.md`. Необязательная стадия на реальных записях: `benchmark_data/README.md`.
...
Полное руководство, включая то, что теперь значит детерминизм между платформами: `docs/REMOTE.md`.
---
...
`variable`	—	variable, metric, parameter, measurement_name, signal_name
`unit`	—	unit, units
Любые другие имена (или экспорт лабораторного прибора с `;` и десятичной запятой) решаются профилем импорта из плагина — см. `examples/lab_device_ru_win1251.csv` и docs/DATA_FORMAT.md.
---
...
В репозитории есть три готовых пакета: `plugin-report-pack-source`, `plugin-lab-pack-source`, `plugin-stress-pack-source`. Справочник: docs/PLUGINS.md.
---
⌨️ Горячие клавиши
...
Выбор метрики на тех же данных, по которым потом делается вывод, завышает ошибку I рода. При полном отсутствии эффекта вероятность того, что хотя бы одна из десяти метрик окажется значимой, равна 0.205, а не 0.05. Помогает раздельная калибровка или выбор метрики до просмотра данных. См. docs/METHODS.md.
...
Веса — экспертное решение, а не измерение; их влияние оценивается анализом чувствительности в `validation/`.
...
Эксперименты, пороги и текущие результаты — в docs/VALIDATION.md, зафиксированы заранее в docs/PREREGISTRATION.md. Независимая перепроверка приветствуется: датасеты, зерна генератора и независимая эталонная реализация лежат в репозитории именно для этого.
...
Ошибки и pull request’ы приветствуются — начните с CONTRIBUTING.md. Главное правило: если меняется определение балла, в том же коммите меняются версия формулы, её хеш и тест `FormulaHash`.
Ошибки и идеи → Issues
Безопасность и проблемы с плагинами → SECURITY.md
Правила общения → CODE_OF_CONDUCT.md
Документация в `docs/` ведётся на английском.
---
📌 Цитирование
Если MVS Analyzer повлиял на опубликованный результат, сошлитесь на метаданные из CITATION.cff и укажите `formula.hash` и `engineVersion` из своего `run_manifest.json` — эта пара точно определяет, как именно получены числа.
---
⚖️ Лицензия
MIT © d1d2dopamine
---
